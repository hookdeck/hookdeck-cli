...
Read or rename the organization your credentials belong to, and manage its API keys.
The API acts on the current organization only — the one the credential in use belongs to.

<!-- GENERATE:org get|org update|org api-key list|org api-key create|org api-key update|org api-key roll|org api-key delete:START -->
- [hookdeck org get](#hookdeck-org-get)
- [hookdeck org update](#hookdeck-org-update)
- [hookdeck org api-key list](#hookdeck-org-api-key-list)
- [hookdeck org api-key create](#hookdeck-org-api-key-create)
- [hookdeck org api-key update](#hookdeck-org-api-key-update)
- [hookdeck org api-key roll](#hookdeck-org-api-key-roll)
- [hookdeck org api-key delete](#hookdeck-org-api-key-delete)

### hookdeck org get

Show the Hookdeck organization your credentials belong to.

**Usage:**

```bash
hookdeck org get [flags]
```
...
Rename the Hookdeck organization your credentials belong to. Name is the only field the API accepts.

**Usage:**

```bash
hookdeck org update [flags]
```
...
| Flag | Type | Description |
|------|------|-------------|
| `--name` | `string` | New organization name (required) |
| `--output` | `string` | Output format: json |

**Examples:**

```bash
$ hookdeck org update --name "Acme Inc"
$ hookdeck org update --name "Acme Inc" --output json
```
### hookdeck org api-key list

List the API keys of the current organization, both organization-scoped and project-scoped. Secrets are never shown; keys are identified by fingerprint.

**Usage:**

```bash
hookdeck org api-key list [flags]
```
...
```
### hookdeck org api-key create

Create an API key in the current organization.
...
```bash
hookdeck org api-key create [flags]
```
...
| Flag | Type | Description |
|------|------|-------------|
| `--label` | `string` | Label for the key (required) |
| `--output` | `string` | Output format: json |
| `--project` | `string` | Scope the key to one project, by ID or name. Omit for an organization key |
| `--scope` | `stringArray` | Permission scope; repeat for several. Defaults to full access. The API is the authority on valid scopes — they are passed through unchecked (default "[]") |

**Examples:**

```bash
$ hookdeck org api-key create --label "CI pipeline"
...
```
### hookdeck org api-key update

Change the permission scopes of an existing API key. The secret is unchanged.

**Usage:**

```bash
hookdeck org api-key update <id> [flags]
```
...
```bash
$ hookdeck org api-key update apk_123 --scope gateway.events.read
$ hookdeck org api-key update apk_123 --scope "*"
```
### hookdeck org api-key roll

Replace an API key with a new one. The old key keeps working for `--delay` seconds,
...
```bash
hookdeck org api-key roll <id> [flags]
```
...
| Flag | Type | Description |
|------|------|-------------|
| `--delay` | `int` | Seconds before the old key stops working (required) (default "0") |
| `--output` | `string` | Output format: json |

**Examples:**

```bash
$ hookdeck org api-key roll apk_123 --delay 3600
```
### hookdeck org api-key delete

Delete an API key. It stops authenticating immediately — anything using it starts
failing at once. To rotate a key with a grace period, use roll instead.

**Usage:**

```bash
hookdeck org api-key delete <id> [flags]
```
...
```bash
$ hookdeck org api-key delete apk_123
$ hookdeck org api-key delete apk_123 --force
```
<!-- GENERATE_END -->
## Projects

<!-- GENERATE:project list|project use|project get|project create|project update|project delete|project custom-domain list|project custom-domain add|project custom-domain remove:START -->
- [hookdeck project list](#hookdeck-project-list)
- [hookdeck project use](#hookdeck-project-use)
- [hookdeck project get](#hookdeck-project-get)
- [hookdeck project create](#hookdeck-project-create)
- [hookdeck project update](#hookdeck-project-update)
- [hookdeck project delete](#hookdeck-project-delete)
- [hookdeck project custom-domain list](#hookdeck-project-custom-domain-list)
- [hookdeck project custom-domain add](#hookdeck-project-custom-domain-add)
- [hookdeck project custom-domain remove](#hookdeck-project-custom-domain-remove)
...
| `--output` | `string` | Output format: json |
| `--private` | `bool` | Create the project as private |
| `--type` | `string` | Project type: gateway or outpost (required) |

**Examples:**

```bash
$ hookdeck project create --name Staging --type gateway
$ hookdeck project create --name "Outpost staging" --type outpost --private
```
### hookdeck project update

Change a project's name or settings.

**Usage:**

```bash
hookdeck project update <id> [flags]
```
...
| `--headers-prefix` | `string` | Prefix applied to forwarded headers |
| `--name` | `string` | New project name |
| `--output` | `string` | Output format: json |
| `--private` | `bool` | Whether the project is private |

**Examples:**

```bash
$ hookdeck project update tm_123 --name "Staging"
```
### hookdeck project delete

Delete a project. Everything in it — connections, sources, destinations, events and
their history — goes with it, and none of it can be recovered.

**Usage:**

```bash
hookdeck project delete <id> [flags]
```
...
```bash
$ hookdeck project delete tm_123
$ hookdeck project delete tm_123 --force
```
### hookdeck project custom-domain list
...
```bash
hookdeck project custom-domain list <project-id> [flags]
```
...
```bash
$ hookdeck project custom-domain list tm_123
```
### hookdeck project custom-domain add
...
```bash
hookdeck project custom-domain add <project-id> <hostname> [flags]
```
...
```bash
$ hookdeck project custom-domain add tm_123 hooks.example.com
```
### hookdeck project custom-domain remove

Remove a custom domain. Traffic to that hostname stops being served.

**Usage:**

```bash
hookdeck project custom-domain remove <project-id> <domain-id> [flags]
```
...
```bash
$ hookdeck project custom-domain remove tm_123 dom_456
```
<!-- GENERATE_END -->
## Local Development
